...
class SMKAttention(nn.Module):

    def __init__(self, channel=3,kernels=[1,3,5,7],reduction=16,group=1,L=32):
        super().__init__()
    ...
    def forward(self, x):
        ...
        ### fuse
        U=sum(conv_outs[1],conv_outs[2]) #bs,c,h,w

        ### reduction channel
        S3=U.mean(-1).mean(-1) #bs,c
        Z3=self.fc(S3) #bs,d

        S1=conv_outs[0].mean(-1).mean(-1)
        Z1=self.fc(S1)

        S7=conv_outs[3].mean(-1).mean(-1)
        Z7=self.fc(S7)
...
class CBAMBlock(nn.Module):
    ...
    def __init__(self, channel=3, reduction=1, kernel_size=49):
        super().__init__()
...
class PSA(nn.Module):
    ...
    def __init__(self, channel=2048, reduction=16, S=1):
        super().__init__()
        self.S = S
...
class SAAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        for x in conv_outs:
            x=x.mean(-1).mean(-1)
            x=self.fs(x)
            all.append(x.view(bs,c,1,1))
        all=torch.stack(all,0)
        attention_weughts=attention_weughts+all
...
class MSCM(nn.Module):
    def __init__(self, out_channel):
        ...
        self.score = nn.Conv2d(out_channel*4, 2048, 1)
...
class SKAttention(nn.Module):
    def __init__(self, channel=3,kernels=[3,5],reduction=16,group=1,L=32):
        super().__init__()
        self.d=max(L,channel//reduction)
        self.convs=nn.ModuleList([])
        for k in kernels:
...
class ResNet(BaseModule):
    ...
    def __init__(self,
        ...
        self.SAAttention=SAAttention()
        self.SKAttention=SKAttention()
        self.PSA=PSA()
        self.CBAMBlock=CBAMBlock()
        self.SMKAttention=SMKAttention()
        self.SRAttention=SRKAttention()
        self.mscm=MSCM(2048)
        self.ra1=RA(1024,1024)
        self.ra2=RA(512,512)
        self.ra3=RA(256,256)
        self.convert1 = nn.Conv2d(2048,1024, 1)
        self.convert2= nn.Conv2d(1024, 512, 1)
        self.convert3 = nn.Conv2d(512, 256, 1)
        self.CoordAtt=CoordAtt(2048,256)
    ...
    def forward(self, x):
        ...
        #x=self.SRAttention(x)
        #x = self.SMKAttention(x)
        if self.deep_stem:
            x = self.stem(x)
        else:
            x = self.conv1(x)
