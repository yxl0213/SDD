...
class ATSSAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        gt_y1 = gt_bboxes[:, 1]
        gt_y3 = gt_bboxes[:, 3]
        gt_points = torch.stack((gt_cx, gt_cy,gt_x0,gt_x2,gt_y1,gt_y3), dim=1)
#        gt_points = torch.stack((gt_cx, gt_cy), dim=1)
        ...
        bboxes_y1 = bboxes[:, 1]
        bboxes_y3 = bboxes[:, 3]
        bboxes_points = torch.stack((bboxes_cx, bboxes_cy,bboxes_x0,bboxes_x2,bboxes_y1,bboxes_y3), dim=1)
#        bboxes_points = torch.stack((bboxes_cx, bboxes_cy), dim=1)
        ...
        for level, bboxes_per_level in enumerate(num_level_bboxes):
            # 考虑长宽和中心点筛选出最接近gt的region proposals, achieve a higher recall rate
